...
class Method2Visualization:
    def __init__(self, model, layer_name, selected_filter):
        self.model = model
        self.model.eval()
...
def run_method1(model, layer_name, filter_index, steps=200):
    activation_dictionary = {}
    module = dict(model.named_modules())[layer_name]
    handle = module.register_forward_hook(amu.layer_hook(activation_dictionary, layer_name))
    input_img = torch.randn(1, 3, 32, 32, requires_grad=True)
...
def run_method2(model, layer_name, filter_index, steps=256):
    vis = Method2Visualization(model, layer_name, filter_index)
    return vis.run(steps)
def run_method3(model, layer_name, filter_index, steps=256):
