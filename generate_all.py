...
import numpy as np
from find_top_filters import get_top_filters, get_model
import methods
import json
from torchvision import transforms

def main(model_type='simple_cnn', model_path='best_model.pth', image_path='frog.jpg', output_dir='results', target_label=None, input_size=32):
    ...
    if target_label is not None and int(target_label) != prediction:
        print(f"VALIDATION_WARNING: Model predicted {prediction}, but ground truth is {target_label}")
    
    print(f"Finding top filters for {image_path} using {model_type}...")
    top_filters = get_top_filters(model_type, image_path, model_path, percentile=90, input_size=input_size)
    block_order = {name: i for i, name in enumerate(layers)}
    ...
    total_steps = len(top_filters) * len(methods_list)
    current_step = 0
    ...
    for f in top_filters:
        ...
        for name, run_fn in methods_list:
            try:
                img = run_fn(model, block, idx, input_size=input_size)
                img_path = os.path.join(output_dir, f"{name}_{block.replace('.','_')}_{idx}.jpg")
                Image.fromarray(img).save(img_path)
                all_images[name][(block, idx)] = f"{name}_{block.replace('.','_')}_{idx}.jpg"
            except Exception as e:
                print(f"Error in {name} for {block} {idx}: {e}")
            
            current_step += 1
            print(f"PROGRESS: {int((current_step / total_steps) * 100)}")
            sys.stdout.flush()
    ...
    for name, _ in methods_list:
        ...
        nodes = []
        links = []
        nodes_by_block = {l: [] for l in layers}
        for f in top_filters:
        ...
        for node in G.nodes():
            ...
            if img_filename:
                img_p = os.path.join(output_dir, img_filename)
...
if __name__ == "__main__":
    ...
    main(args.model_type, args.model_path, args.image, args.output, args.label, args.input_size)
