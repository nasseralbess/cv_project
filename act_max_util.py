...
def act_max(network, 
    ...
    for k in range(steps):
        ...
        if conv:
            (layer_out[0][unit].sum()).backward(retain_graph=True)
        elif single_neuron is not None:
            layer_out[tuple(single_neuron)].backward(retain_graph=True)
        else:
        ...
        if verbose:
            if conv:
                print('step: ', k, 'activation: ', (layer_out[0][unit]).sum())
            else:
        ...
        if conv:
            if best_activation < (layer_out[0][unit]).sum():
                best_activation = (layer_out[0][unit]).sum()
                best_img = input
        elif single_neuron is not None:
            if best_activation < layer_out[tuple(single_neuron)]:
                best_activation = layer_out[tuple(single_neuron)]
                best_img = input
        else:
